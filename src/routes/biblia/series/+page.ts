...
export type BibliaSeriesIndexPageData = {
  series: BibliaSeriesListItem[];
};

export const load: PageLoad = async ({ fetch }): Promise<BibliaSeriesIndexPageData> => {
  ...
  if (!response.ok) {
    throw error(response.status, "No se pudo cargar la lista de series.");
  }
